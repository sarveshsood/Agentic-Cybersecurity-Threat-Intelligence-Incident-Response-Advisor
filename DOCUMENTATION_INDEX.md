# Documentation Index (root)

Canonical detailed index: **[docs/DOCUMENTATION_INDEX.md](docs/DOCUMENTATION_INDEX.md)**

## v1.0 Enterprise Demo Pack — quick map

| Area                            | Path                                                                                                  |
|---------------------------------|-------------------------------------------------------------------------------------------------------|
| Master roadmap (done + planned) | [ROADMAP.md](ROADMAP.md) · in-app **Roadmap** page                                                    |
| Product vision (Command Center) | [docs/product/VISION.md](docs/product/VISION.md)                                                      |
| Enterprise review               | [docs/ENTERPRISE_REVIEW.md](docs/ENTERPRISE_REVIEW.md) · [ENTERPRISE_REVIEW.md](ENTERPRISE_REVIEW.md) |
| Presentations                   | [presentation/](presentation/)                                                                        |
| Diagrams                        | [diagrams/](diagrams/)                                                                                |
| Developer experience            | [docs/dx/](docs/dx/)                                                                                  |
| Operations                      | [docs/operations/](docs/operations/)                                                                  |
| AI governance                   | [docs/ai-governance/](docs/ai-governance/)                                                            |
| Compliance                      | [docs/compliance/](docs/compliance/)                                                                  |
| Business                        | [docs/business/](docs/business/)                                                                      |
| Deployments                     | [deployments/](deployments/)                                                                          |
| API collections                 | [api/](api/)                                                                                          |
| Examples                        | [examples/](examples/)                                                                                |
| Benchmarks                      | [benchmarks/](benchmarks/)                                                                            |
| Samples / demo                  | [samples/](samples/)                                                                                  |
| Monitoring                      | [monitoring/](monitoring/)                                                                            |
| One-command start               | [scripts/start-demo.ps1](scripts/start-demo.ps1) · [scripts/start-demo.sh](scripts/start-demo.sh)     |
