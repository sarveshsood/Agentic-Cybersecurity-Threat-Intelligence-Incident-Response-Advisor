...
/** Alias — same 7 providers as layout LIVE INTEL badge */
...
/** Primary nav tabs — one section visible at a time */
const SETTINGS_TABS = [
    {id: "llm", label: "LLM", icon: Cpu, sectionKey: "llm"},
    {id: "pipeline", label: "Detection", icon: Sliders, sectionKey: "pipeline"},
    {id: "threat_intel", label: "Threat intel", icon: Key, sectionKey: "threat_intel"},
    {id: "notifications", label: "Alerts", icon: Bell, sectionKey: "notifications"},
    {id: "access", label: "Access & data", icon: Shield, sectionKey: "security"},
    {id: "ui", label: "UI prefs", icon: Desktop, sectionKey: "ui"},
];

const VALID_TAB_IDS = new Set(SETTINGS_TABS.map((t) => t.id));
const DEFAULT_TAB = "llm";

/** Map form/validation field keys → category tab (for badges + jump-to). */
const FIELD_TO_TAB = {
    llm_provider: "llm",
    llm_model: "llm",
    llm_temperature: "llm",
...
/** Ops keys that define factory vs recommended profile (exclude free-form email). */
...
/** Derive active profile badge from form values (not just last button click). */
...
function apiErrorDetail(e) {
    ...
    if (status === 404) {
        return "API route not found (404). Is the ACTIRA backend running on REACT_APP_BACKEND_URL with the latest server.py?";
    }
...
/** Shared body for field help (hover only). */
...
/** Section panel — same shell as Dashboard / Knowledge (design-system Panel) */
function Panel({title, icon: Icon, sectionKey, children, subtitle}) {
    ...
    return (
        <DsPanel
            ...
            testid={sectionKey ? `section-${sectionKey}` : undefined}
            actions={
                section ? (
                    <FieldTip meta={section} fieldKey={`section-${sectionKey}`} tipKey={`section-${sectionKey}`}/>
                ) : null
            }
...
/** Compact field: label + tip + input + warning only (no always-on chips) */
function Field({
                   label,
                   fieldKey,
                   children,
...
/** Collapsed health strip — expand only when needed; click field → jump to tab */
...
/** Full-width field grids — use horizontal space on laptop / ultrawide */
...
function formFromSettings(d) {
    return {
        ...
        llm_temperature: d.llm_temperature,
        llm_token_budget_monthly: d.llm_token_budget_monthly,
        grounding_threshold: d.grounding_threshold,
        hitl_severity_min: d.hitl_severity_min,
        auto_approve_grounding_min: d.auto_approve_grounding_min,
        correlation_window_minutes: d.correlation_window_minutes,
        session_timeout_hours: d.session_timeout_hours,
        failed_login_lockout: d.failed_login_lockout,
        incident_retention_days: d.incident_retention_days,
        enrichment_cache_ttl_hours: d.enrichment_cache_ttl_hours,
...
export default function Settings() {
    ...
    const setActiveTab = useCallback(
        (id) => {
            ...
            // Scroll tab panel into view after switch
    ...
    /** Per-tab error/warning counts so badges stay in sync with form validation */
    ...
    /** Profile badge derived from current form — stays synced after load/edit/save */
    ...
    const hydrate = (d) => {
        setSettings(d);
        setForm(formFromSettings(d));
    };

    useEffect(() => {
        api.get("/settings").then((r) => hydrate(r.data));
    }, []);

    // Keep URL tab param canonical when invalid/missing aliases are used
    useEffect(() => {
        const raw = searchParams.get("tab");
        if (!raw) return;
        const norm = normalizeTabId(raw);
        if (raw !== norm) {
            setSearchParams(
                (prev) => {
                    const p = new URLSearchParams(prev);
                    if (norm === DEFAULT_TAB) p.delete("tab");
                    else p.set("tab", norm);
                    return p;
                },
                {replace: true},
    ...
    const save = async () => {
        ...
        try {
            ...
            // Keep browser UI prefs in sync when dirty (one click "save all")
    ...
    return (
        <div data-testid="settings-page" className="pb-4">
            ...
            Hover <Info size={12} className="inline text-primary/80"/> for field help · secrets stay blank after load
                ...
                actions={
                    <div className="flex flex-wrap items-center gap-2" data-testid="settings-profile-tabs">
                        ...
                        <span
                            data-testid={profileTab === "custom" ? "profile-custom" : "profile-status"}
                            title="Detected from current field values (recommended / factory / custom mix)"
                            className={`text-[10px] uppercase tracking-[0.08em] font-semibold px-2.5 py-1 rounded-md border ${
                                profileTab === "recommended"
                                    ? "text-success border-[var(--success-border)] bg-success-soft"
                                    : profileTab === "factory"
                                        ? "text-warning border-[var(--warning-border)] bg-warning-soft"
            ...
            {/* Category tabs — enterprise horizontal nav, synced with ?tab= */}
            <div
                className="flex flex-wrap gap-1 mb-5 p-1.5 rounded-card border border-border bg-card shadow-sm sticky top-14 z-20"
                ...
                {SETTINGS_TABS.map(({id, label, icon: Icon}) => {
                    ...
                    return (
                        <button
                            ...
                            className={`inline-flex items-center gap-2 px-3.5 py-2 rounded-lg text-[13px] font-medium transition-colors min-h-[2.25rem] ${
                                active
                                    ? "bg-primary text-primary-foreground shadow-sm"
                                    : "text-muted-foreground hover:text-foreground hover:bg-muted"
                            }`}
                        >
                            <Icon size={15} weight={active ? "bold" : "regular"} className="shrink-0"/>
                            <span>{label}</span>
                            {badgeN > 0 && (
                                <span
                                    className={`min-w-[1.15rem] h-[1.15rem] px-1 rounded-md text-[9px] font-mono grid place-items-center ${
                                        counts.error
                                            ? active
            ...
            >
                ...
                {activeTab === "llm" && (
                    <div className="grid grid-cols-1 xl:grid-cols-12 gap-4">
                        <div className="xl:col-span-8 space-y-4">
                            <Panel title="LLM provider" icon={Cpu} sectionKey="llm">
                                ...
                                <button
                                    ...
                                    className="text-[11px] text-primary/80 hover:text-primary inline-flex items-center gap-1"
                                ...
                                {showLlmAdvanced && (
                                    <div className={`${FIELD_GRID_2} pt-1`}>
                                        <Field
                                            label="Temperature"
                    ...
                    {(settings.llm_usage.tokens_used ?? 0).toLocaleString()}
                        ...
                        <aside className="xl:col-span-4 space-y-4">
                            <Panel title="Active stack" icon={Cpu} subtitle="Runtime snapshot from saved settings">
                            ...
                            <Panel title="Ops profile" icon={Sparkle} subtitle="Detected from current field values">
                                <div
                ...
                {activeTab === "pipeline" && (
                    <div className="grid grid-cols-1 xl:grid-cols-12 gap-4">
                        <div className="xl:col-span-8">
                            <Panel title="Detection & review" icon={Sliders} sectionKey="pipeline">
                                <div className={FIELD_GRID_2}>
                                    <Field
                        ...
                        <aside className="xl:col-span-4 space-y-4">
                            <Panel title="HiTL summary" icon={Sliders} subtitle="How review gates behave">
                                <dl className="space-y-3 text-sm">
                ...
                {activeTab === "threat_intel" && (
                    <div className="grid grid-cols-1 xl:grid-cols-12 gap-4">
                        <div className="xl:col-span-8">
                            <Panel title="Threat intelligence" icon={Key} sectionKey="threat_intel">
                                ...
                                <div className="rounded-card border border-border overflow-hidden">
                                    <table className="soc-table text-[12px]">
                                        ...
                                        <tbody>
                                        {TI_KEYS.map(([label, field, hasKey]) => {
                                ...
                                {/* Hidden inputs keep testids discoverable when not editing — only active edit has key-* */}
                            </Panel>
                        </div>
                        <aside className="xl:col-span-4 space-y-4">
                            <Panel title="Intel coverage" icon={Key}
                ...
                {activeTab === "notifications" && (
                    <Panel title="Alerts" icon={Bell} sectionKey="notifications">
                ...
                {activeTab === "access" && (
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                        <Panel title="Security" icon={Shield} sectionKey="security">
                            <div className={FIELD_GRID_2}>
                                <Field
                        ...
                        <Panel title="Data retention" icon={HardDrives} sectionKey="retention">
                            <div className={FIELD_GRID_2}>
                                <Field
                ...
                {/* ——— UI preferences (browser-local; no Mongo secrets) ——— */}
                {activeTab === "ui" && (
                    <div className="space-y-4" data-testid="settings-ui-prefs">
                        <Panel title="Dashboard & tables" icon={Desktop} sectionKey="ui">
                            <p className="text-[12px] text-muted-foreground max-w-3xl">
                                Presentation knobs stored in this browser (localStorage). Hover{" "}
                                <Info size={11} className="inline text-primary/80"/> for default vs recommended;
                                green <span className="text-success font-medium">rec</span> means the field matches
                                the ACTIRA UI profile.
                            </p>
                            ...
                            <div className={`${FIELD_GRID} pt-4 border-t border-border`}>
                                <Field
                            ...
                            <div className="flex flex-wrap gap-2 pt-1 border-t border-border">
                                <button
                                    type="button"
                                    data-testid="ui-prefs-recommended"
                                    className="inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-semibold border border-[var(--success-border)] text-success hover:bg-success-soft transition-colors"
                                    onClick={() => {
                                ...
                                >
                                    <Sparkle size={12} weight="fill"/>
                                    Apply recommended UI
                                </button>
                                <button
                                    type="button"
                                    data-testid="ui-prefs-save"
                                    className="soc-btn-primary !text-xs !px-3 !py-1.5 !h-auto"
                                    onClick={persistUiPrefs}
                                >
                                    Save UI preferences
                                    {uiPrefsDirty ? " · unsaved" : ""}
                                </button>
                                <button
                                    type="button"
                                    data-testid="ui-prefs-reset"
                                    className="soc-btn-secondary !text-xs !px-3 !py-1.5 !h-auto"
                                    onClick={() => {
            ...
            {/* Sticky save bar — context-aware for ops vs UI tab */}
            <div
                className="mt-6 w-full flex flex-wrap items-center justify-end gap-2.5 sticky bottom-4 z-10 bg-card/95 backdrop-blur-md border border-border rounded-card px-4 py-3 shadow-md">
                {activeTab === "ui" ? (
                    <>
            <span className="text-[12px] text-muted-foreground mr-auto">
              UI prefs are browser-local{uiPrefsDirty ? " · unsaved changes" : ""}
            </span>
                        <button
                            type="button"
                            data-testid="ui-prefs-save-sticky"
                    ...
                    <>
                        {hasBlockingErrors && (
                            <span className="text-[12px] text-error mr-auto flex items-center gap-1.5">
                ...
                >
                  Go to issue
                        ...
                        {!hasBlockingErrors && uiPrefsDirty && (
