...
import {ArrowsClockwise, Books, ChartBar, Database, MagnifyingGlass} from "@phosphor-icons/react";
...
export default function Knowledge() {
    ...
    const search = async () => {
        ...
        try {
            ...
            setResults(hits);
            setSearchHistory((h) => [
                {q, mode, n: hits.length, at: Date.now()},
    ...
    const ingest = async () => {
        if (!ingestText.trim()) {
            toast.error("Paste document text first");
            return;
        }
        setIngestBusy(true);
        try {
            ...
            toast.success(`Ingested ${r.data?.doc?.id || "document"}`);
            setIngestText("");
            loadStatus();
        } catch (e) {
            toast.error(e?.userMessage || e?.response?.data?.detail || "Ingest failed");
        } finally {
            setIngestBusy(false);
        }
    };
    ...
    const reindex = async () => {
        ...
        try {
            ...
            toast.error(e?.response?.data?.detail || "Reindex failed (admin only)");
        } finally {
    ...
    const trainLora = async () => {
        ...
        try {
            ...
            toast.error(e?.response?.data?.detail || e?.userMessage || "LoRA train failed (admin only)");
        } finally {
            setLoraBusy(false);
        }
    };
    ...
    return (
        <div data-testid="kb-page">
            ...
            {scoreBars.length > 0 && (
                <div className="soc-card p-4 mb-5" data-testid="kb-score-chart">
        ...
        <span className="soc-label flex items-center gap-1.5 text-primary/90">
          <Database size={12}/> Vector store
            ...
            <div className="space-y-3">
                {results.map((r) => (
                    <div key={r.id} className="soc-card p-4" data-testid={`kb-hit-${r.id}`}>
                        <div className="flex items-center justify-between mb-1 gap-2">
                            <div className="flex items-center gap-2 flex-wrap">
                                <span className="citation-chip" title="Document id">{r.id}</span>
                    ...
                    rerank {Number(r.rerank_score).toFixed(2)}
                                ...
                                <span className="font-mono text-[10px] text-success" title="Fused ranking score">
