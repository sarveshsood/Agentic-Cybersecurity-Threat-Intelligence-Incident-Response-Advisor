...
import {ChartBar, Database, Fingerprint, Globe, Scales, TrendUp, Warning,} from "@phosphor-icons/react";
...
function StatCell({label, value, accent = "primary", testid, helpKey}) {
    const cls = {
        ...
        // legacy aliases (map to enterprise tokens)
        cyan: "text-primary",
        violet: "text-primary",
    }[accent] || "text-primary";
...
export default function Analytics() {
    ...
    const cumulativeTimeline = useMemo(() => {
        ...
        return data.timeline.map((row) => {
            // Prefer explicit total if present; else sum severities
    ...
    return (
        <div data-testid="analytics-page">
            <PageHeader
                ...
                tip={
                    <HelpTip title="Security Analytics" testid="tip-analytics-page">
                        <p>EDA over incidents, IoCs, ATT&CK techniques, and retrieval quality for the selected
                            window.</p>
                ...
                subtitle={
                    <>
                        Last {days} days · {t.incidents} incidents · {t.events_processed} events · {critPct}% critical
                    </>
                }
                actions={
            ...
            {/* Retrieval comparison — BM25 vs LanceDB hybrid (identification) */}
            {showRetrievalPanel ? (
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 mb-4">
                <Card testid="chart-techniques-card">
                ...
                <Card testid="chart-ioctypes">
                    ...
                    <ResponsiveContainer width="100%" height={260}>
                        <BarChart data={data.ioc_type_distribution}>
                            <CartesianGrid strokeDasharray="2 4" stroke={chart.grid}/>
                            <XAxis dataKey="type" tick={{...chart.tick, fontFamily: "IBM Plex Mono"}}/>
                            <YAxis tick={chart.tick}/>
                            <Tooltip contentStyle={chart.contentStyle}/>
