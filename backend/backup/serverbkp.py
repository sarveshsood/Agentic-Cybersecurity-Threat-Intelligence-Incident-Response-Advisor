...
from auth import get_current_user, set_user_loader
from core.database import client, db  # noqa: F401 — re-exported for tests
...
async def lifespan(app: FastAPI):
    try:
        ...
        try:
            from auth_throttle import ensure_throttle_indexes
        ...
        try:
            from llm_usage import set_usage_db

            set_usage_db(db)
        except Exception as ue:
            logger.warning("llm_usage bind skipped: %s", ue)
        try:
            from job_status import purge_old_sidecars
            from notifications import purge_old_outbox
            from auth_throttle import purge_stale_throttle_docs
            from retention import purge_from_settings
...
async def rate_limit_middleware(request: Request, call_next):
    ...
    if request.method == "POST" and path in _AUTH_RATE_LIMIT_PATHS:
        from auth_throttle import rate_limit_allow
