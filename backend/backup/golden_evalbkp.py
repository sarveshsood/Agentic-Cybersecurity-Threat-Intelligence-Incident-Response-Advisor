...
from attack_mapping import technique_ids_for_eval
...
DEFAULT_THRESHOLDS = {
    ...
    "max_mean_latency_s": 7.0,  # Increased from 5.0s to accommodate local real-time server runs
}
...
def run_offline_case(
    ...
    # Pure mock enrichment — force_mock ignores Settings/env API keys (CI must stay offline)
    enriched: List[IoC] = [enrich_ioc(i, {}, force_mock=True) for i in iocs]
    # Offline: text-only inference (no CES events unless caller passes them later)
    ...
    if force_template_playbook:
        ...
        # BM25-only: avoid dense/rerank paths that may call external APIs
        ...
        # A-G1: optional live LLM playbook (not used in CI; requires keys)
        import asyncio
        ...
        try:
            ...
            # already in event loop — use template fallback
    ...
    # No CES correlation in offline slice — use IoC/technique severity only
    ...
    return {
        ...
        # Include parent roots so gold labels like T1110 still match T1110.001 predictions
...
def load_golden_dataset(path: Optional[Path] = None) -> List[Dict[str, Any]]:
    """Load cases from tests/golden/dataset.json (or GOLDEN_DATASET_PATH)."""
    import os
