...
async def list_roadmap(
    ...
    # Prevent browser caching of dynamic roadmap lists (Fixes 304 Not Modified responses)
...
async def delete_roadmap_item(
    ...
    """Delete a specific roadmap item by ID or MongoDB ObjectId with fallback."""
    await svc.ensure_roadmap_seeded()

    # Try deleting by custom string ID first
    result = await db.roadmap.delete_one({"id": item_id})

    # If not found, try matching by the title slug or ObjectId
    ...
    if result.deleted_count == 0:
        raise HTTPException(status_code=404, detail=f"Roadmap item not found for identifier: {item_id}")
...
async def deduplicate_roadmap(user=Depends(require_roles("admin"))):
    """Automatically remove duplicate roadmap items by title, keeping the richest/seed entry."""
    ...
    for doc in all_items:
        ...
        if title not in seen:
            ...
            # Prefer seed IDs over custom stubs (rm-custom-...)
            ...
            if is_cur_custom and not is_ext_custom:
                ...
                # Compare description & task length if both are same type
    ...
    await svc.audit(user, "roadmap.deduplicate", "roadmap", "all", {"deleted_count": len(deleted_ids)})
    ...
    if force:
        for item in ROADMAP_SEED:
            ...
            if existing:
                ...
                d["created_at"] = now
                # Use upsert=True to completely prevent duplicate key errors on race conditions or re-seeds
                await db.roadmap.update_one(
                    {"id": d["id"]},
