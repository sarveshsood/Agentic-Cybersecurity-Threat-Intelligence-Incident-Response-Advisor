...
async def kpis(user=Depends(get_current_user)):
    ...
    # Mean grounding + MTTR proxy (created → reviewed) + severity/status mixes
    ...
    for d in docs:
        ...
        if created and reviewed:
            try:
                c = created if isinstance(created, datetime) else datetime.fromisoformat(
                    str(created).replace("Z", "+00:00"))
                r = reviewed if isinstance(reviewed, datetime) else datetime.fromisoformat(
                    str(reviewed).replace("Z", "+00:00"))
                if c.tzinfo is None:
    ...
    mean_mttr_hours = round(sum(review_hours) / len(review_hours), 2) if review_hours else None
    ...
    # ATT&CK heatmap counts (technique + parent rollup for sub-techniques)
    tech_counts: dict = {}
    for inc in docs:
...
async def analytics(
    ...
    """EDA-style aggregations over recent incidents (Analytics page)."""
...
async def analytics_retrieval_compare(
    ...
    """Compare BM25 vs LanceDB dense vs hybrid (+ re-rank) for identification.

    Offline golden Q→doc pairs — no Mongo required for the comparison itself.
    Used by Analytics → Retrieval comparison panel.
    """
